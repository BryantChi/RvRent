@foreach ($sp_filter ?? [] as $key => $sp)
    @foreach ($sp ?? [] as $item)
    <?php
     $src = route('car_rent_s2', ['rvm_id' => $key, 'spid' => $item['id']]);
     $func = "modelsSelect('{$src}', 'sp')";
     $onclick = (bool) $item['reserved'] ? '' : $func;
     $click_txt = (bool) $item['reserved'] ? '預定額滿' : '選擇';
     $click_btn_style = (bool) $item['reserved'] ? 'btn-danger' : 'btn-primary3';
    ?>
                ...
                <div class="card-body p-0">
                    ...
                    <div class="content px-4 py-4">
                        ...
                        <ul class="list-unstyled d-flex justify-content-between align-items-center border-top mt-3 pt-3 mb-0">
                            <li class="text-muted"><a href="javascript:void(0)" class="btn {{$click_btn_style}}" onclick="{{ $onclick }}">{{$click_txt}} <i data-feather="arrow-right" class="fea icon-sm"></i></a></li>
